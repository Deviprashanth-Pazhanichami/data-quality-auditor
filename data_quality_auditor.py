...
class DataQualityAuditor:
    ...
    def auto_clean(self, cap_outliers=True, standardize_low_cardinality_text=True):
        ...
        # 5b. Standardize mixed date formats to ISO 8601 (YYYY-MM-DD)
        for col in text_cols:
            series = df[col].dropna().astype(str)
            if len(series) == 0 or not self._looks_like_dates(series):
                continue
            parsed = pd.to_datetime(df[col], errors="coerce", format="mixed")
            n_parsed = parsed.notna().sum()
            n_original_non_null = df[col].notna().sum()
            # Only apply if we could confidently parse the large majority of values,
            # so we never silently corrupt a column that only coincidentally looks date-like.
            if n_original_non_null > 0 and (n_parsed / n_original_non_null) >= 0.9:
                new_col = parsed.dt.strftime("%Y-%m-%d")
                new_col = new_col.where(parsed.notna(), df[col])  # keep original where parsing failed
                changed = (new_col.astype(str) != df[col].astype(str)).sum()
                if changed > 0:
                    df[col] = new_col
                    log.append(f"Standardized {changed} date value(s) in '{col}' to ISO format (YYYY-MM-DD).")
